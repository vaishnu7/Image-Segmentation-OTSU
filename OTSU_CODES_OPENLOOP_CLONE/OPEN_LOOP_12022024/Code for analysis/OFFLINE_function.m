...
for i=1:n_timeframes
    ...
    for j= 1:n_CHANNELS % updating each channel numbers to fileNamesBCKGRND{j} e.g. 5 in this experiment
    ...
    % [~,~,~, ~, ~, singleMASK]=Mask_n_Track(Parameters.fluoeval.CHANNEL{Parameters.fluoeval.mask_channel},...
...
if (Parameters.crop_flag==1) 
    if (Parameters.crop_drawn==1) 
        ...
        [~,crop] = imcrop(imadjust(imread(image))); % imcrop: creates an interactive crop image tool and returns the cropped image from the above "image" 
...
for type_i = 1:Parameters.fluoeval.n_Channels
    ...
    ctrl_position = strcat('Position', ...
        ...
        '--C', pad(num2str(type_i - 1), 2, 'left', '0'), '.tif'); % this is how the image is identified by the name e.g. Position011-t01-C00.tif
    %---------------------------------------------------------------
    ctrl_position = fullfile(Parameters.Video.dir_img , ctrl_position);
...
%NB: THE MATLAB COMAND CAN BE CHANGED ACCORDING TO THE CELLS TYPE
...
%% CODES LINE TO USE FOR MY EXPERIMENTS
%% Images REXH2B12 (2i test su ph) best result, to optimaze with Matlab app
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% PHASE CONTRAST
filter_size=3;
radius=3;
...
BW = imfill(BW, 'holes'); % fills holes in the input binary image BW. 
...
for i=1:n_timeframes
    ...
    %% controlla questo canale che sia mcherry negli exp o rendilo generale con variabile fuori
     daw=imread(fileNamesBCKGRND{Parameters.sulpho});
...
%          Im_Green = imcrop(imread(Parameters.fluoeval.CHANNEL{2}),Parameters.fluoeval.crop);
         BackGround = imcrop(imread(Parameters.fluoeval.BCKGRND_CHANNEL{1}),Parameters.fluoeval.CropRectBCKG);
    else
        Ph_image = imread(Parameters.fluoeval.CHANNEL{1});
%          Im_Green = (imread(Parameters.fluoeval.CHANNEL{2}));
       BackGround = (imread(Parameters.fluoeval.BCKGRND_CHANNEL{2}));
    ...
    if tracking_flag==1
      ...
      figure(fig1)
      title('Open-loop Naïve-Primed transition')
...
%       imshow(imadjust(cropped_image{4}))
        imshow(imadjust(cropped_image{Parameters.sulpho})); % changed to this because image was not proper
...
%       imread(fileNamesBCKGRND{4});
    ...
    subplot(4,4,6);
        daw=imread(fileNamesBCKGRND{Parameters.sulpho});
        daw=(double(daw-min_daw))/double(max_daw-min_daw);
        imshow(4*daw);
        title(strcat('DAW Sulpho'))

%     subplot(4,3,8)
% %         BackGround2 = imcrop(imread(Parameters.fluoeval.BCKGRND_CHANNEL{4}),Parameters.fluoeval.CropRectBCKG)
% %         imshow(imadjust(BackGround2))
% 
%           sulpho_crop=imcrop(imread(Parameters.fluoeval.BCKGRND_CHANNEL{4}),Parameters.fluoeval.CropRectBCKG)
%           sulpho_crop=(double(sulpho_crop-min_sulpho_crop))/double(max_sulpho_crop-min_sulpho_crop);
%         imshow(sulpho_crop);
%         title(strcat('Crop sulpho')) 
        
    
%% Modificato with Normalized fluo <GFP> 

   subplot(4,1,3);
     title(strcat('Frame n. ',num2str(i-1)));
     %% Choose the input to plot
...
%           yticklabels({'-(2i+LIF)','+(2i+LIF)'})
    ...
    subplot(4,1,4);
      % Normalizzazione sulla fluorescenza calcolata dal rapporto di
      %% GFP/IRP sulla fase di calibrazione
      cb=3; %value to moidify according to the frame of the calibration phase
