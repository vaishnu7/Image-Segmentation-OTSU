...
% % subplot(3,2,6);
%% normalizzazione tra zero ed 1
%  Value normlaized according to the old experiment between one and zero (Irene old exp)

    % normalizationp=(outputArg1.Nuclei_Normalized_Fluorescence_Channel_A_A(:,cf)-min(outputArg1.Nuclei_Normalized_Fluorescence_Channel_A_A(:,cf)))/(max(outputArg1.Nuclei_Normalized_Fluorescence_Channel_A_A(1:cb,cf))- min(outputArg1.Nuclei_Normalized_Fluorescence_Channel_A_A(:,cf)))
    % plot(0:60:(Parameters.n_timeframes-1)*60,normalizationp,'linewidth',1,'color','[0.2 0.6 0]')
    %  hold on 
    %  legend ('<GFP>','location','best','fontsize',6)
    %  ylabel('Fluo [A.u.]','fontsize',12);
    %  xlabel('Time [min]','fontsize',12);
    %  xlim([0 (Parameters.n_timeframes-1)*60]);
    %  set(gca,'FontSize',12)
